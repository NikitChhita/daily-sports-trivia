  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const quizRes = await fetch("https://api.daily-sports-trivia.com/quiz/today");
        const quizData = await quizRes.json();
        setQuiz(quizData);

        // fetch correct answers now that quiz is finished
        const answersRes = await fetch(
          `https://api.daily-sports-trivia.com/quiz/answers/${quizData.dq_id}`,
        );
        const correctData = await answersRes.json();
        setCorrectAnswers(correctData);

        if (isLoggedIn) {
          const statsRes = await fetch("https://api.daily-sports-trivia.com/stats/me", {
          ...
          if (statsRes.ok) {
            ...
            if (lastResult) {
              ...
              const myAnswersRes = await fetch(
                `https://api.daily-sports-trivia.com/quiz/my-answers/${quizData.dq_id}`,
                { headers: { Authorization: `Bearer ${token}` } },
              );
